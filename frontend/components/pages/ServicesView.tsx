  ...
  return (
    <main className="min-h-screen bg-white px-5 py-12 text-[#07111F] lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <h1 className="mt-4 text-4xl font-black text-[#0B1E33]">{t("servicesPage.title")}</h1>
        <p className="mt-4 max-w-3xl text-[#4B5563]">{t("servicesPage.subtitle")}</p>

        <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {services.map((service) => (
            <article key={service.slug} className="rounded-xl border border-[#E5E7EB] bg-white p-6 shadow-sm">
              <h2 className="text-xl font-black">{service.title}</h2>
              <p className="mt-2 text-sm leading-7 text-[#4B5563]">{service.description}</p>
              <ul className="mt-4 list-disc space-y-1 pl-5 text-sm text-[#374151]">
