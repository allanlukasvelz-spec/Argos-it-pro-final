/**
 * Asset registry + TLS tables (Phase 2). Idempotent boot ensure.
 */
const DDL = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_assets_hostname ON assets(organization_id, lower(hostname)) WHERE hostname IS NOT NULL`,
...
async function ensureAssetsTables(pool) {
...
function isAllowedAssetType(type) {
  return ASSET_TYPES.includes(String(type || "").toUpperCase());
}
